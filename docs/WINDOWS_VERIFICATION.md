...
| C 窗口系统 | C2 菜单工作区落位、C3 位置记忆、C6 Win32 样式、C7 重力落地 | C1/C4/C5 多屏、DPI、全屏、虚拟桌面 |
| D 发布 | D1、D3/D4 的产物检查、D5 注册表开关 | D2 图标观感、D5 登录后真实自启、D6 干净系统/Defender |
...
| G7 | 气泡 | 无白线；随宠物移动；不改变宠物窗口尺寸；进场为"从下往上 8px + 淡入"160ms，无系统侧滑动；对话窗口为"从上往下 8px + 淡入"180ms | DWM 过渡在首次显示前关闭（2026-09-16，B9 断言）；观感待复测 |
...
## H. 阶段 7 回归重点（2026-09-16：自启 / 多屏位置记忆 / 重力）

`-Full` 已经自动覆盖 T4（自启注册表）、C3（物理像素位置记忆）、C7（重力落地）和 B14（屏幕外
回落）。下面这几条只是把自动化无法判断的“真实系统行为”补上：

| # | 检查 | 关注点 | 状态 |
|---|---|---|---|
| H1 | 设置 →「启动」勾选开机自启动，然后取消 | `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 出现/消失 `Petsona` 值，内容为 `"<exe 路径>"`；任务管理器 → 启动应用能看到它 | 待实测（T4 已自动验证注册表读写） |
| H2 | 拖动宠物到副屏，重启 | 精确回到副屏原位置；混合 DPI 下不偏移 | 待实测（C3 已自动验证物理像素保存） |
| H3 | 副屏右键托盘/宠物 | 菜单出现在该屏工作区内，不越界、不跳屏 | 待实测（T3 已自动验证工作区夹取） |
| H4 | 拔掉副屏（或把 `startPosition` 改成屏幕外坐标）后重启 | 宠物回到最近可见工作区，不会“消失” | 待实测（B14 已自动覆盖屏幕外坐标） |
| H5 | 开启重力，把宠物拖到半空松手 | 平滑下落、停在工作区底部、播放一次 `jumping`；拖动/自动行走时不下落 | 待实测（C7 已自动验证落点与落地动画） |
