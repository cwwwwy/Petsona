...
```powershell
# 先跑完整门禁（fmt / clippy / 87 项 Rust 测试 / release + 27 项 native smoke + 打包结构）
...
覆盖：Rust workspace 门禁（fmt / clippy / **87 项测试** / release）+ `petsona_ffi.dll` +
dotnet locked restore / build / format / **44 项测试** + **native smoke 27 项**
...
N25 `action:"clear"` 解除粘滞的协议状态、N26 工作区夹取、
N27 贴底时编辑条侧挂）+ 打包结构与内容检查。Release 构建后还有 FFI DLL SHA256 哈希守卫。
...
| W6 | 气泡 / 编辑条 / Composer | 协议 message 显示气泡；悬浮编辑条打开输入框并聚焦；Enter 发送、Shift+Enter 换行、Esc 关闭且保留草稿；中文 IME 组合不误发 | 打开路径端到端 + N18 聚焦通过；**2026-09-21 人工通过**（旧圆钮路径的 Enter / Esc / IME 行为已确认；新条状入口待 W31 复测） |
...
| W29 | 宠物边界 | 拖到任务栏上沿、屏幕左/右/上边缘，松手后宠物窗口仍完整在工作区内；重启恢复位置也不会越界 | 自动 N26；多屏/混合 DPI 待 W12 解冻后人工 |
| W30 | 气泡计时 | 气泡淡入；底部进度条随剩余时间递减；鼠标停留时进度暂停且气泡不消失，移开后从剩余时间继续；新气泡重新开始 | 共享 runtime 测试 + EngineClient 暂停测试；观感待人工 |
| W31 | 编辑条 | 宠物下方优先显示小矩形条；靠近任务栏时自动移到左右空间中较大的一侧并旋转；悬停约 220ms 展开并聚焦，点击立即打开 | 自动 N18/N27；悬停时序待人工 |
| W32 | Composer 跟随与外观 | 无标题栏/边框、无快捷键提示，只显示输入区和发送按钮；拖动宠物时输入框始终跟随，并在下方/左右侧挂间保持稳定 | 自动 N18/N27 覆盖打开与侧挂；拖动跟随即时性和观感待人工 |
...
| W15 | 设置页收束（settings-consolidation） | 六页卡片布局（宠物 / 外观与交互 / 人格 / 记忆 / 连接与问候 / 系统）；改动即时生效（无保存按钮）；删除宠物 / 清空记忆有确认；系统页可打开数据与日志目录、显示版本 | 自动：dotnet 44/44（含 `UpdateGreetingConfig`、气泡暂停、OverlayLayout）、`verify-windows.ps1 -Full` exit 0、smoke 27/27 ×2；**2026-09-21 用户实机通过**（后续设置页微调见执行记录「下一批」） |
| W12 | 多屏 / 混合 DPI / 自动隐藏任务栏 / 重力 / 自动活动提醒 / 透明度 / 协议设置界面 / 宠物图标托盘化 / 影子动画 | —— | **暂缓**（工作区夹取本体已由 N26 实现；混合 DPI、自动隐藏任务栏和其余项目仍按计划 v1.1 §3.1 暂缓，解冻需用户确认） |
...
**原生线（当前产品线）**：`scripts\windows-smoke.ps1` **27 项** `N1`–`N27`（启动 / 窗口 / 宠物 /
样式 / 编辑条 / 托盘 / 协议 / TTL / `action:clear` / 穿透 / 单实例 / 空库首启 / 端口释放 /
webp / 点击 / 动画帧 / 托盘菜单 / 聚焦 / 拖动动画 / 注视 / 光标 / 工作区夹取 / 侧挂），随后是打包结构检查。
